...
namespace Orders.Backend.Data
{
    public class SeedDb
    {
        ...
        private readonly IFileStorage _fileStorage; //Inyecto el FileStorage para que suba imagenes
        ...
        public async Task SeedAsync()
        {
            ...
            await CheckRolesAsync();//Crea nuevos roles y verifica si están en el sistema.
            await CheckProductsAsync(); //Crea nuevos productos.
        ...
        //Carga la información de paises estados y cieudades desde el fichero de carga.
        ...
        //Si no existen paises, crea dos paises que son España y Francia
        ...
        //Que checkee si existen los roles Admin y User. Si no existen los crea.
            ...
            string email, string phone, string address, string image, UserType userType)
        {
            ...
            if (user == null) //Si no existe el usuario, lo crea.
            {
                var city = await _context.Cities.FirstOrDefaultAsync(x => x.Name == "Bilbao");
                ...
                //Mandamos la imagen al BlobStorage de Azure.
                var filePath = $"{Environment.CurrentDirectory}\\Images\\Users\\{image}"; //Obtenemos la imagen de la carpeta Images/Users
                var fileBytes = File.ReadAllBytes(filePath); //Leemos la imagen y la convertimos en un array de Bytes
                var imagePath = await _fileStorage.SaveFileAsync(fileBytes, "jpg", "users"); //Lo mandamos a la carpeta users de nuestro blobStorage
                ...
                user = new User
                {
                    ...
                    City = city,
                    UserType = userType,
                    Photo = imagePath, //Le cargamos la foto al usuario.
                };

                await _usersUnitOfWork.AddUserAsync(user, "123456");//Le pasa el usuario y el password.
                await _usersUnitOfWork.AddUserToRoleAsync(user, userType.ToString());//Asigna el usuario al Tipo de usuario.
        ...
        //Añade los productos asociados a las categorías y a la foto asociada.
        ...
        private async Task AddProductAsync(string name, decimal price, float stock, List<string> categories, List<string> images)
        {
            ...
            foreach (var categoryName in categories) //Busca la categoría del producto y por cada categoría se la adiciona al producto.
            {
            ...
            foreach (string? image in images) //Busca las imagenes y por cada imagen que encuentre se la asocia al producto.
            {
                //Mandamos la imagen al BlobStorage de Azure.
